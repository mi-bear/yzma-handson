// この例は練習問題 4 ( ファイル入力への対応 ) の解答例。
// ../summarize/ は標準入力からしかテキストを受け取れなかったが、ここでは
// -file フラグでファイルパスを指定すると、そのファイルの内容を処理対象にする。
// 入力元の切り替えを readInput という純粋関数に切り出してあるので、
// llama や標準入出力に触れずにユニットテストできる。
//
// 使い方:
//
//	echo "要約したい文章" | go run . -mode summary
//	go run . -mode summary -file article.txt
package main
...
func main() {
	...
	file := flag.String("file", "", "入力テキストファイルのパス ( 省略時は標準入力から読む )")
	flag.Parse()
	...
	input, err := readInput(*file, os.Stdin)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
...
// readInput は処理対象のテキストを取得する。
// file が空でなければそのファイルを読み、空なら stdin から読む。
// どちらの経路でも、読み込んだ内容が空なら分かりやすいエラーにする。
func readInput(file string, stdin io.Reader) ([]byte, error) {
	...
	if file != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("ファイルの読み込みに失敗 ( %s ): %w", file, err)
		}
	} else {
		data, err = io.ReadAll(stdin)
		if err != nil {
			return nil, fmt.Errorf("標準入力の読み込みに失敗: %w", err)
		}
	}
	if len(data) == 0 {
		return nil, fmt.Errorf("入力が空です。処理するテキストを渡してください")
	}
	return data, nil
}
